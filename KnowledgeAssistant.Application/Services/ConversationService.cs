...
public class ConversationService
{
    ...
    public async IAsyncEnumerable<string> GenerateAssistantMessageAsync(Guid conversationId, string message, string provider, string model, MessageSource source, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (conversation.Provider != provider || conversation.SelectedModelId != modelId)
        {
            conversation.Provider = provider;
            conversation.SelectedModelId = modelId;
            await _repository.UpdateSelectedModelAsync(conversationId, provider, modelId, cancellationToken);
        }
        ...
        await foreach (var token in modelGateway.StreamAsync(model, messages, cancellationToken))
        {
            buffer.Append(token);
            yield return token;
        }
        ...
        {
            await ClassifyTopicAsync(conversationId, messages, model, cancellationToken);
        }
    }
