...
public class ConversationsService : IMessageServiceSubscriber
{
    ...
    private async void SendUserMessageReceived(MessageBase message)
    {
        if (message is SendUserMessageRequest request)
        {
            ...
            if (_conversation != null && ShouldGenerateTitle(_conversation))
            {
                ...
                _messageService.Publish(new GenerateTitleRequest(request.Prompt, _conversation.Provider ?? _currentProvider, _conversation.SelectedModel ?? _currentModel, _conversation.Id));
            }

            _messageService.Publish(new SendPromptRequest(request.Prompt, _conversation?.Provider ?? _currentProvider, _conversation?.SelectedModel ?? _currentModel, "user", _conversation?.Id));
        }
    }
    ...
    private void SendToolPromptReceived(MessageBase message)
    {
        if (message is SendToolPromptRequest request)
        {
            ...
            _messageService.Publish(new SendPromptRequest(request.Context, _conversation.Provider ?? _currentProvider, _conversation.SelectedModel ?? _currentModel, "tool", _conversation.Id, request.SystemPrompt));
        }
    }
    ...
    private void UpdateSelectedModelReceived(MessageBase message)
    {
        ...
        {
            _currentProvider = request.SelectedProvider;
            _currentModel = request.SelectedModel;
            if (_conversation != null)
            {
                _conversation.Provider = request.SelectedProvider;
